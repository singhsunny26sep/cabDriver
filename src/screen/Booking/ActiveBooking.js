...
export default function ActiveBooking({navigation}) {
      ...
      if (socketServices.isConnected()) {
        ...
        socketServices.on(`${activeTab}_booking_list`, data => {
          const filteredData = data.filter(
            item => item.bookingStatus === 'waiting for pickup',
          );
  ...
  const handleTabChange = typeValue => {
    ...
    socketServices.emit(`${typeValue}_booking`, {
      type: 'waiting for pickup',
    });
  ...
  const handleAcceptRide = async bookingId => {
    try {
      ...
      showToast('error', 'Error', 'Failed to accept the ride');
    }
  };
